...
pub async fn run() {
    ...
    // log::info!("steps: {:#?}", probabilities);
    // log::info!("steps: {:#?}", probabilities.len());

    let mut probabilities = probabilities.iter().enumerate().collect::<Vec<_>>();

    probabilities.sort_unstable_by(|a, b| b.1.partial_cmp(a.1).unwrap());

    log::info!("Infered result: {}", probabilities[0].0);
}
...
async fn execute_gpu() -> Option<HashMap<String, OutputTensor>> {
    ...
    let session = wonnx::Session::from_bytes(include_bytes!("./models/opt-mnist.onnx")).await.unwrap();
...
//         log::info!("time: main: {:#?}", time_pre_compute.elapsed());
//     }
...
pub fn load_image() -> ndarray::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::Dim<[usize; 4]>> {
    ...
    // .to_rgb8();
    let image_buffer: ImageBuffer<Rgb<u8>, Vec<u8>> = image::load_from_memory(include_bytes!("../7.jpg"))
    .unwrap()
    .resize_exact(28, 28, FilterType::Nearest)
    .to_rgb8();
